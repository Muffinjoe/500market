...
<head>
    ...
    <style>
        ...
        .news-item {{ display: flex; gap: 14px; padding: 14px; border: 1px solid var(--border); border-radius: 10px; text-decoration: none; color: var(--text); transition: all 0.15s; }}
        .news-item:hover {{ border-color: var(--blue); background: var(--bg-secondary); }}
        .news-img {{ width: 80px; height: 60px; border-radius: 6px; object-fit: cover; flex-shrink: 0; background: var(--bg-secondary); }}
        ...
        @media (max-width: 640px) {{ .news-img {{ width: 60px; height: 45px; }} }}
...
<body>
    ...
    (function(){{
        ...
        .then(r=>r.json()).then(articles=>{{
            ...
            const badImg=['yahoo','yimg.com','logo','favicon','default','placeholder','icon'];
            function isGoodImg(url){{if(!url)return false;const l=url.toLowerCase();return!badImg.some(b=>l.includes(b))&&(l.endsWith('.jpg')||l.endsWith('.jpeg')||l.endsWith('.png')||l.endsWith('.webp')||l.includes('/image'));}}
            ...
            container.innerHTML=top.map(a=>{{
                ...
                const hasImg=isGoodImg(a.image);
                const img=hasImg?'<img class="news-img" src="'+a.image+'" alt="" onerror="this.style.display=\\'none\\'">':'';
                return '<a href="'+a.url+'" target="_blank" rel="noopener" class="news-item">'+img+'<div class="news-content"><div class="news-headline">'+a.headline+'</div><div class="news-meta"><span class="news-source">'+a.source+'</span> · '+timeStr+'</div></div></a>';
            }}).join('');
