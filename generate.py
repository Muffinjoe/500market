...
<body>
    ...
    if(dlBtn)dlBtn.addEventListener("click",function(){{
        const S={json.dumps({"ticker":stock["ticker"],"name":stock["name"],"price":stock["price"],"change1d":stock["change1d"],"change7d":stock["change7d"],"changeYtd":stock["changeYtd"],"marketCap":stock["marketCap"],"pe":stock["pe"],"sector":stock["sector"],"rank":stock["rank"]})};
        const c=document.createElement("canvas");c.width=600;c.height=340;const x=c.getContext("2d");
        x.fillStyle="#1a1a2e";x.beginPath();x.roundRect(0,0,600,340,16);x.fill();
        x.fillStyle="#3861FB";x.beginPath();x.roundRect(0,0,600,56,[16,16,0,0]);x.fill();
        x.fillStyle="#fff";x.font="bold 18px system-ui";x.fillText("500Market",20,36);
        x.font="12px system-ui";x.fillStyle="rgba(255,255,255,0.6)";x.textAlign="right";
        x.fillText(new Date().toLocaleDateString("en-US",{{month:"long",day:"numeric",year:"numeric"}}),580,36);x.textAlign="left";
        x.fillStyle="#fff";x.font="bold 28px system-ui";x.fillText(S.ticker,20,100);
        x.font="16px system-ui";x.fillStyle="#8b949e";x.fillText(S.name,20,124);
        x.font="bold 36px system-ui";x.fillStyle="#fff";x.fillText("$"+S.price.toLocaleString("en-US",{{minimumFractionDigits:2}}),20,175);
        const cc=S.change1d>=0?"#16c784":"#ea3943";const cs=S.change1d>=0?"+":"";
        x.font="bold 20px system-ui";x.fillStyle=cc;x.fillText(cs+S.change1d.toFixed(2)+"%",20,205);
        function fmc(n){{if(n>=1e12)return"$"+(n/1e12).toFixed(1)+"T";if(n>=1e9)return"$"+(n/1e9).toFixed(1)+"B";return"$"+(n/1e6).toFixed(0)+"M";}}
        const st=[["Market Cap",fmc(S.marketCap)],["P/E",S.pe?S.pe.toFixed(1):"N/A"],["7d",(S.change7d>=0?"+":"")+S.change7d.toFixed(2)+"%"],["YTD",(S.changeYtd>=0?"+":"")+S.changeYtd.toFixed(2)+"%"],["Sector",S.sector],["Rank","#"+S.rank]];
        st.forEach(function(s,i){{const col=i%3;const row=Math.floor(i/3);const px=20+col*190;const py=240+row*44;
        x.fillStyle="#8b949e";x.font="11px system-ui";x.fillText(s[0],px,py);
        x.fillStyle=(s[0]==="7d"||s[0]==="YTD")?((parseFloat(s[1])>=0)?"#16c784":"#ea3943"):"#e4e6eb";
        x.font="bold 14px system-ui";x.fillText(s[1],px,py+17);}});
        x.fillStyle="#2d3748";x.fillRect(0,320,600,20);x.fillStyle="#8b949e";x.font="10px system-ui";x.textAlign="center";
        x.fillText("500market.com",300,334);x.textAlign="left";
