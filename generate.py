...
<body>
    ...
    (function(){{
        ...
        .then(r=>r.json()).then(articles=>{{
            ...
            const name='{stock["name"]}'.toLowerCase();const tick=STOCK.ticker.toLowerCase();
            const generic=['top movers','s&p500','s&p 500','stay informed','market update','stocks to watch','sector update','wall street'];
            const relevant=articles.filter(a=>{{const h=a.headline.toLowerCase();const isGeneric=generic.some(g=>h.includes(g))&&!h.includes(tick)&&!h.includes(name);return!isGeneric;}}).sort((a,b)=>b.datetime-a.datetime).slice(0,8);
